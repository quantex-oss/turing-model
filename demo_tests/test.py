...
class Bond(IR, InstrumentBase, metaclass=ABCMeta):
    ...
    freq_type: Union[str, FrequencyType] = None  # 付息频率
    accrual_type: Union[str, DayCountType] = None  # 计息类型
    par: float = None  # 面值
    cpn_type: Union[str, CouponType] = None  # 付息方式
    ...
    def __post_init__(self):
        ...
        if self.cpn_type:
            if self.cpn_type == 'zero coupon':
                self.cpn_type = CouponType.ZERO_COUPON
            elif self.cpn_type == 'discount':
                self.cpn_type = CouponType.DISCOUNT
            elif self.cpn_type == 'coupon-carrying':
                self.cpn_type = CouponType.COUPON_CARRYING
            elif isinstance(self.cpn_type, CouponType):
                pass
            else:
                raise TuringError('Please check the input of cpn_type')
        if self.freq_type:
            if self.freq_type == '每年付息':
                self.freq_type = FrequencyType.ANNUAL
            elif self.freq_type == '半年付息':
                self.freq_type = FrequencyType.SEMI_ANNUAL
            elif self.freq_type == '4个月一次':
                self.freq_type = FrequencyType.TRI_ANNUAL
            elif self.freq_type == '按季付息':
                self.freq_type = FrequencyType.QUARTERLY
            elif self.freq_type == '按月付息':
                self.freq_type = FrequencyType.MONTHLY
            elif isinstance(self.freq_type, FrequencyType):
                pass
            else:
        ...
        if self.accrual_type:
            if self.accrual_type == 'ACT/365':
                self.accrual_type = DayCountType.ACT_365L
            elif self.accrual_type == 'ACT/ACT':
                self.accrual_type = DayCountType.ACT_ACT_ISDA
            elif self.accrual_type == 'ACT/360':
                self.accrual_type = DayCountType.ACT_360
            elif self.accrual_type == '30/360':
                self.accrual_type = DayCountType.THIRTY_E_360
            elif self.accrual_type == 'ACT/365F':
                self.accrual_type = DayCountType.ACT_365F
            elif isinstance(self.accrual_type, DayCountType):
                pass
            else:
