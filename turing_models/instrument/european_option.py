...
class EuropeanOption:
    ...
    @property
    def texp(self) -> float:
        return (self.expiration_date - self.value_date_) / gDaysInYear

    @property
    def r(self) -> float:
        df = self.discount_curve.df(self.expiration_date)
        return -np.log(df)/self.texp

    @property
    def q(self) -> float:
        dq = self.dividend_curve.df(self.expiration_date)
    ...
    def price(self) -> float:
        return bsValue(*self.params())

    def eq_delta(self) -> float:
        return bsDelta(*self.params())

    def eq_gamma(self) -> float:
        return bsGamma(*self.params())

    def eq_vega(self) -> float:
        return bsVega(*self.params())

    def eq_theta(self) -> float:
        return bsTheta(*self.params())

    def eq_rho(self) -> float:
        return bsRho(*self.params())

    def eq_rho_q(self) -> float:
        return bsPsi(*self.params())
