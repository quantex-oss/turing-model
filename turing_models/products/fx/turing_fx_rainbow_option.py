...
from turing_models.turingutils.turing_math import N, M
...
from turing_models.products.equity.turing_equity_option import TuringEquityOption
...
class TuringFXRainbowOptionTypes(Enum):
    CALL_ON_MAXIMUM = 1
    PUT_ON_MAXIMUM = 2
    CALL_ON_MINIMUM = 3
    PUT_ON_MINIMUM = 4
...
def payoffValue(s, payoffTypeValue, payoffParams):
    ...
    if payoffTypeValue == TuringFXRainbowOptionTypes.CALL_ON_MINIMUM.value:
        k = payoffParams[0]
        payoff = np.maximum(np.min(s, axis=1) - k, 0.0)
    elif payoffTypeValue == TuringFXRainbowOptionTypes.CALL_ON_MAXIMUM.value:
        k = payoffParams[0]
        payoff = np.maximum(np.max(s, axis=1) - k, 0.0)
    elif payoffTypeValue == TuringFXRainbowOptionTypes.PUT_ON_MINIMUM.value:
        k = payoffParams[0]
        payoff = np.maximum(k - np.min(s, axis=1), 0.0)
    elif payoffTypeValue == TuringFXRainbowOptionTypes.PUT_ON_MAXIMUM.value:
        k = payoffParams[0]
        payoff = np.maximum(k - np.max(s, axis=1), 0.0)
    elif payoffTypeValue == TuringFXRainbowOptionTypes.CALL_ON_NTH.value:
        n = payoffParams[0]
        k = payoffParams[1]
        ssorted = np.sort(s)
        assetn = ssorted[:, -n]
        payoff = np.maximum(assetn - k, 0.0)
    elif payoffTypeValue == TuringFXRainbowOptionTypes.PUT_ON_NTH.value:
        n = payoffParams[0]
        k = payoffParams[1]
        ssorted = np.sort(s)
        assetn = ssorted[:, -n]
...
class TuringRainbowOption(TuringEquityOption):

    def __init__(self,
                 expiryDate: TuringDate,
                 payoffType: TuringFXRainbowOptionTypes,
...
###############################################################################
    ...
    def validatePayoff(self, payoffType, payoffParams, numAssets):
        ...
        if payoffType == TuringFXRainbowOptionTypes.CALL_ON_MINIMUM:
            numParams = 1
        elif payoffType == TuringFXRainbowOptionTypes.CALL_ON_MAXIMUM:
            numParams = 1
        elif payoffType == TuringFXRainbowOptionTypes.PUT_ON_MINIMUM:
            numParams = 1
        elif payoffType == TuringFXRainbowOptionTypes.PUT_ON_MAXIMUM:
            numParams = 1
        elif payoffType == TuringFXRainbowOptionTypes.CALL_ON_NTH:
            numParams = 2
        elif payoffType == TuringFXRainbowOptionTypes.PUT_ON_NTH:
            numParams = 2
        else:
            raise TuringError("Unknown payoff type")
        ...
        if payoffType == TuringFXRainbowOptionTypes.CALL_ON_NTH \
                or payoffType == TuringFXRainbowOptionTypes.PUT_ON_NTH:
            n = payoffParams[0]
...
###############################################################################
    ...
    def value(self,
        ...
        if self._payoffType == TuringFXRainbowOptionTypes.CALL_ON_MAXIMUM:
            v = s1 * dq1 * M(y1, d, rho1)+s2*dq2*M(y2, -d + v*sqrtt, rho2) \
                - k * df * (1.0 - M(-y1 + v1*np.sqrt(t), -y2 + v2*sqrtt, rho))
        elif self._payoffType == TuringFXRainbowOptionTypes.CALL_ON_MINIMUM:
            v = s1*dq1*M(y1, -d, -rho1) + s2 * dq2 * M(y2, d-v*np.sqrt(t), -rho2)\
                - k * df * M(y1 - v1 * np.sqrt(t), y2 - v2 * np.sqrt(t), rho)
        elif self._payoffType == TuringFXRainbowOptionTypes.PUT_ON_MAXIMUM:
            ...
            v = k * df - cmax1 + cmax2
        elif self._payoffType == TuringFXRainbowOptionTypes.PUT_ON_MINIMUM:
...
, -rho2) - k * df * M(y1 - v1 * sqrtt, y2 - v2 * sqrtt, rho)
            ...
            raise TuringError("Unsupported FX Rainbow option type")
