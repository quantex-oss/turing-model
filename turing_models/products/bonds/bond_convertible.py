...
###############################################################################
    ...
    def __repr__(self):
        ...
        s += to_string("CONVERSION RATIO", self._conversionRatio)
        s += to_string("START CONVERT DATE", self._startConvertDate)
        s += to_string("CALL", "DATES")

        for i in range(0, len(self._callDates)):
            s += to_string(self._callDates[i],
                           self._callPrices[i])

        s += to_string("PUT", "DATES")

        for i in range(0, len(self._putDates)):
            s += to_string(self._putDates[i],
                           self._putPrices[i])
