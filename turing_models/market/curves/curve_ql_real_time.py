...
class Shibor3M:
    ...
    def _build(self, deposit_mkt_data, swap_mkt_data, fixing_data, today):
        ...
        for depo_tenor in deposit_mkt_data.columns:
            ix = ql.IborIndex(self.name, ql.Period(depo_tenor), depo_delay, ql.CNYCurrency(), calendar, convention, eom, float_daycount)
            hp = ql.DepositRateHelper(ql.QuoteHandle(ql.SimpleQuote(deposit_mkt_data.loc[0, depo_tenor])), ix)
            deposit_helpers.append(hp)

        swap_index = ql.IborIndex(self.name, ql.Period('3M'), swap_delay, ql.CNYCurrency(), calendar, convention, eom, float_daycount)
        swap_helpers = [ ql.SwapRateHelper(ql.QuoteHandle(ql.SimpleQuote(swap_mkt_data.loc[0, tenor])), ql.Period(tenor), calendar,
        ...
        index = ql.IborIndex(self.name, ql.Period('3M'), swap_delay, ql.CNYCurrency(), calendar, convention, eom, float_daycount, curve_ts)
...
class LPR1Y:
    ...
    def _build(self, deposit_mkt_data, swap_mkt_data, fixing_data, today):
        ...
        swap_index = ql.IborIndex(name, ql.Period('3M'), swap_delay, ql.CNYCurrency(), calendar, convention, eom, float_daycount)
        swap_helpers = [ ql.SwapRateHelper(ql.QuoteHandle(ql.SimpleQuote(swap_mkt_data.loc[0, tenor])), ql.Period(tenor), calendar,
                                           ql.Quarterly, convention, fixed_daycount, swap_index) for tenor in swap_mkt_data.columns ]
        ...
        index = ql.IborIndex(name, ql.Period('3M'), swap_delay, ql.CNYCurrency(), calendar, convention, eom, float_daycount, curve_ts)
...
'''
def build_curves_libor(curve_data, fixing_data, today, calendar, convention, daycount):

    dates = [calendar.advance(today, ql.Period(tenor)) for tenor in curve_data['Tenor']]
    zeros = [zero/100 for zero in curve_data['ZeroRate']]
...
class CNYShibor3M:
    ...
    def _build(self, curve_data, fixing_data, today):
        ...
        index = ql.IborIndex(self.name, ql.Period('3M'), swap_delay, ql.CNYCurrency(), calendar, convention, eom, daycount, curve_ts)
...
class RfCurve:
    ...
    def __init__(self, spot, fwd_data, USDLibor_data, today, calendar, daycount):

        self.spot = spot
        self.fwd_data = fwd_data.copy()
        self.USDLibor_data = USDLibor_data.copy()
        self.today = today
        self.calendar = calendar
        self.daycount = daycount
        self.curve = self._build(self.spot, self.fwd_data, self.USDLibor_data, self.today,
                                             self.calendar, self.daycount)


    def _build(self, spot, fwd_data, USDLibor_data, today, calendar, daycount):

        #fwd_dates = [today] + [calendar.advance(today, ql.Period(fwd_data.loc[ix, ['Tenor']][0])) for
        #                       ix in fwd_data.index]
        #print(fwd_data)
        merge_data = pd.merge(fwd_data,USDLibor_data, on = "Date")
        merge_dates = [dt.datetime.strptime(x,"%Y/%m/%d") for x in merge_data["Date"]]
        merge_dates = [ql.Date(d.day,d.month,d.year) for d in merge_dates]
        merge_data["Date"] = merge_dates
        rf = []
        for i,j,k in zip(merge_data["Date"],merge_data["Spread"],merge_data["Rate"]):
            tao = daycount.yearFraction(today,i)
            rf.append(math.log(math.exp(k/100*tao)/(j/spot+1))/tao)
        dates = [today] + merge_dates
        zeros = [0] + rf
        rf_curve = ql.ZeroCurve(dates, zeros, daycount, calendar)
