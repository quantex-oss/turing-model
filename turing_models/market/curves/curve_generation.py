...
class DomDiscountCurveGen:
    ...
    def __init__(self,
        ...
        shibor_curve = getattr(self, 'shibor_curve_data') or TuringDB.shibor_curve()
        shibor_deposit_tenors = shibor_curve['tenor'][:5]
        shibor_deposit_rates = shibor_curve['rate'][:5]
        shibor_3m_curve = getattr(self, 'irs_curve_data') or TuringDB.irs_curve(curve_type='Shibor3M')['Shibor3M']
...
class ForDiscountCurveGen:
    ...
    def __init__(self,
        ...
        future_data = getattr(self, 'swap_curve_data') or TuringDB.swap_curve(asset_id=fx_asset_id)[fx_asset_id]
        future_tenors = future_data['tenor']
        future_quotes = future_data['swap_point']
        self.future_dates = value_date.addYears(future_tenors)
        exchange_rate = getattr(self, 'exchange_rate_data') or TuringDB.exchange_rate(symbol=currency_pair)[currency_pair]
        self.fwd_dfs = []
