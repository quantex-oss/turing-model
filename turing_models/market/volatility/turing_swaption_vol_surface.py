...
from turing_models.products.fx.turing_fx_mkt_conventions import TuringFXATMMethod
from turing_models.products.fx.turing_fx_mkt_conventions import TuringFXDeltaMethod
...
from turing_models.models.turing_model_volatility_fns import volFunctionBloomberg
from turing_models.models.turing_model_volatility_fns import volFunctionSVI
from turing_models.models.turing_model_volatility_fns import volFunctionSSVI
from turing_models.models.turing_model_sabr import volFunctionSABR
from turing_models.models.turing_model_sabr import volFunctionSABR_BETA_HALF
from turing_models.models.turing_model_sabr import volFunctionSABR_BETA_ONE

from turing_models.models.turing_model_volatility_fns import TuringVolFunctionTypes

from turing_models.turingutils.turing_math import norminvcdf

from turing_models.models.turing_model_black_scholes_analytical import bsValue
from turing_models.products.fx.turing_fx_vanilla_option import fastDelta
from turing_models.turingutils.turing_distribution import TuringDistribution

from turing_models.turingutils.turing_solvers_1d import newton_secant
from turing_models.turingutils.turing_solvers_nm import nelder_mead
from turing_models.turingutils.turing_global_types import TuringSolverTypes
...
def volFunction(volFunctionTypeValue, params, f, k, t):
    ...
    if volFunctionTypeValue == TuringVolFunctionTypes.CLARK.value:
        ...
        vol = volFunctionClark(params, f, k, t)
        return vol
    elif volFunctionTypeValue == TuringVolFunctionTypes.SVI.value:
        vol = volFunctionSVI(params, f, k, t)
        return vol
    elif volFunctionTypeValue == TuringVolFunctionTypes.SSVI.value:
...
class TuringSwaptionVolSurface():
    ...
    Check out TuringVolFunctionTypes. Visualising the volatility curve is useful.
    ...
    def __init__(self,
        ...
        if len(strikeGrid.shape) != 2:
            raise TuringError("Strike grid must be a 2D grid of values")

        if len(volatilityGrid.shape) != 2:
            raise TuringError("Volatility grid must be a 2D grid of values")
            
        if len(strikeGrid) != len(volatilityGrid):
            raise TuringError("Strike grid and volatility grid must have same size")

        if len(strikeGrid[0]) != len(volatilityGrid[0]):
            raise TuringError("Strike grid and volatility grid must have same size")

        if len(expiryDates) != len(volatilityGrid[0]):
            raise TuringError("Expiry dates not same size as volatility grid")
...
###############################################################################
    ...
    def _buildVolSurface(self, finSolverType=TuringSolverTypes.NELDER_MEAD):
        ...
        if self._volatilityFunctionType == TuringVolFunctionTypes.CLARK:
            ...
            numParameters = 5
        elif self._volatilityFunctionType == TuringVolFunctionTypes.SVI:
            numParameters = 5
        elif self._volatilityFunctionType == TuringVolFunctionTypes.SSVI:
            numParameters = 5
        else:
