...
from turing_models.instruments.fx.fx_quanto_range_accrual_ql import FXQuantoRangeAccrualQL
...
class FXQuantoRangeAccrual(FXOption):
    ...
    range_up: float = None
    range_down: float = None
    in_coupon: float = None
    out_coupon: float = None
    ...
    def __post_init__(self):
        ...
        self.accrual_start_ql = turingdate_to_qldate(self.accrual_start)
        self.accrual_end_ql = turingdate_to_qldate(self.accrual_end)
        self.observe_start_date_ql = turingdate_to_qldate(self.observe_start_date)
        self.observe_end_date_ql = turingdate_to_qldate(self.observe_end_date)
        ...
        if self.spot_f_d and not self.strike:
            self.strike = self.spot_f_d
        if self.strike and not self.range_up:
            self.range_up = self.strike * 1.03
        if self.strike and not self.range_down:
            self.range_down = self.strike * 0.97
        # 2
        ...
        # 6
        twopair_fixings = pd.read_excel(self.mkt_file, 'fixings')
        self.fixing_data_fd = pd.DataFrame({'Fixing': twopair_fixings['Series2']})
        self.fixing_data_fd.index = [ql.Date(d.date().day, d.date().month, d.date().year) for d in twopair_fixings['Date']]

        if self.foreign_name and self.domestic_name and self.notional_currency \
           and self.observe_start_date_ql and self.observe_end_date_ql and self.accrual_start_ql \
           and self.accrual_end_ql and self.range_up and self.range_down and self.in_coupon \
           and self.out_coupon and self.notional and self.trade_direction:
            self.option = FXQuantoRangeAccrualQL(f_ccy=self.foreign_name,
                                                 d_ccy=self.domestic_name,
                                                 q_ccy=self.notional_currency,
                                                 obsStart=self.observe_start_date_ql,
                                                 obsEnd=self.observe_end_date_ql,
                                                 accrualStart=self.accrual_start_ql,
                                                 accrualEnd=self.accrual_end_ql,
                                                 rangeUp=self.range_up,
                                                 rangeDown=self.range_down,
                                                 inCoupon=self.in_coupon,
                                                 outCoupon=self.out_coupon,
    ...
    def volatility_surface_d_q(self):
        if self.d_q_symbol:
            return FXVolSurfaceGen(value_date=self.value_date_,
                                   ...
                                   foreign_discount_curve=self.domestic_discount_curve_f_d,
    ...
    def resolve_param(self):
        ...
        if not self.product_type:
            setattr(self, 'product_type', 'QUANTO_RANGE_ACCRUAL')
        self.__post_init__()
