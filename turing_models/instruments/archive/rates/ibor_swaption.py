...
from turing_models.instruments.archive.rates.ibor_swap import TuringIborSwap
...
###############################################################################
    ...
    def __repr__(self):
        ...
        s += to_string("SETTLEMENT DATE", self._settlementDate)
        s += to_string("EXERCISE DATE", self._exerciseDate)
        s += to_string("SWAP FIXED LEG TYPE", str(self._fixedLegType))
        s += to_string("SWAP MATURITY DATE", self._maturityDate)
        s += to_string("SWAP NOTIONAL", self._notional)
        s += to_string("FIXED COUPON", self._fixedCoupon * 100)
        s += to_string("FIXED FREQUENCY", str(self._fixedFrequencyType))
        s += to_string("FIXED DAY COUNT", str(self._fixedDayCountType))
        s += to_string("FLOAT FREQUENCY", str(self._floatFrequencyType))
        s += to_string("FLOAT DAY COUNT", str(self._floatDayCountType))

        if self._pv01 is not None:
            s += to_string("PV01", self._pv01)
            s += to_string("FWD SWAP RATE", self._fwdSwapRate * 100)
            s += to_string("FWD DF TO EXPIRY", self._forwardDf, "")
