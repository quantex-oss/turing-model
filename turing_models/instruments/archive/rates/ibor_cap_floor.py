...
###############################################################################
    ...
    def __repr__(self):
        ...
        s += to_string("STRIKE COUPON", self._strikeRate * 100)
        s += to_string("OPTION TYPE", str(self._optionType))
        s += to_string("FREQUENCY", str(self._freqType))
        s += to_string("DAY COUNT", str(self._dayCountType), "")
        return s
